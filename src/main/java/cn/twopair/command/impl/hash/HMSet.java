...
/**
 * 实现Redis的HMSET兼容命令，向Hash批量写入字段和值。
 *
 * <p>该命令从Redis 4.0开始已弃用，新业务应使用支持多组字段的HSET，
 * 但Redis的AOF Rewrite仍可能使用HMSET恢复Hash数据。
 *
 * @author ljj
 */
public class HMSet implements WriteCommand {
	 ...
	 * 解析HMSET命令参数。
	 *
	 * @param array 命令数组，格式为HMSET key field value [field value ...]
	 * @throws IllegalArgumentException 当参数数量、类型或内容不合法时抛出
	 */
	@Override
	public void setContent(Resp[] array) {
		if (array == null || array.length < 4 || (array.length - 2) % 2 != 0) {
			throw new IllegalArgumentException("HMSET命令需要key和至少一组field、value参数");
		}

		for (int i = 1; i < array.length; i++) {
			if (!(array[i] instanceof BulkString)) {
				throw new IllegalArgumentException("HMSET的key、field和value必须是BulkString");
			}
		}
		...
		if (parsedKey == null || parsedKey.getByteArray() == null) {
			throw new IllegalArgumentException("HMSET的key不能是NIL");
		}
		...
		for (int i = 2; i < array.length; i += 2) {
			...
			if (field == null || field.getByteArray() == null || value == null || value.getByteArray() == null) {
				throw new IllegalArgumentException("HMSET的field和value不能是NIL");
			}

			// 重复field保留最后一个value，与Redis命令语义一致。
	...
	public Resp handle(RedisCore redisCore) {
		redisCore.putHashFields(key, fields);
