...
> Stage 8 更新：`stage/08-aof-performance` 已完成 `EVERYSEC` 后台刷盘，并追加同版本 A/B 验证数据；Stage 7 原始数据保留作为优化前基线。
...
`mvn test`：**168 个用例 / 43 个测试类，全部通过（0 失败、0 错误、0 跳过）**。覆盖 RESP 编解码（半包/非法输入）、四种数据结构、并发与过期语义、25 个命令的参数校验与执行、EmbeddedChannel 集成、AOF 追加、刷盘策略与重放（含尾部截断自愈）。
...
### 3.4 Stage 8 EVERYSEC 优化验证

为避免不同代码版本和运行状态干扰，本轮使用当前 `stage/08-aof-performance` 代码进行同环境 A/B 测试。两个服务都使用全新临时 AOF 文件、`WARN` 日志级别及 `-n 10000 -c 50 -d 64 -t set,get`；`ALWAYS` 预热 1000 次后正式测量，`EVERYSEC` 充分预热 50000 次后连续测量三轮并取中位数。

| 当前版本策略 | SET ops/s | SET p50 | GET ops/s | GET p50 |
|---|---:|---:|---:|---:|
| `ALWAYS` | 272.11 | 146.047 ms | 133,333.33 | 0.183 ms |
| `EVERYSEC`（三轮中位数） | 112,359.55 | 0.431 ms | 178,571.42 | 0.151 ms |

结论：`EVERYSEC` 将 SET 吞吐提高约 **413 倍**，p50 从 146.047 ms 降到 0.431 ms。优化前，每条写命令都在 Netty EventLoop 上等待 `force`；优化后，请求线程只追加到操作系统文件缓存，独立的 `redis-aof-fsync` 线程每秒刷盘一次，因此写吞吐不再由单次 fsync 延迟决定。代价是操作系统崩溃或断电时最多可能丢失约 1 秒尚未刷盘的数据，正常关闭仍会执行最终刷盘。

### 3.5 kill -9进程强杀恢复验证

本轮在全新临时目录启动 `EVERYSEC` 服务，先写入 `crash:flushed` 并等待 2 秒，再写入 `crash:unforced` 后立即使用 `kill -9` 强杀 JVM。重启服务后，AOF 文件的 124 字节被完整解析，两条 SET 命令均成功重放，两条数据均可读取。

这个结果验证了进程突然终止后AOF仍可重放，但**不能证明EVERYSEC不存在一秒数据丢失窗口**。`FileChannel.write` 返回时数据已经进入操作系统页缓存，`kill -9` 只终止JVM，不会清空内核缓存；尚未 `force` 的第二条命令因此通常仍能恢复。真正的持久性窗口需要通过断电、操作系统崩溃、虚拟机故障注入或可控存储测试验证。单元测试则使用 `AofStorage` 测试替身，确定性验证请求线程不立即刷盘、后台线程定时刷盘以及正常关闭最终刷盘。
...
| everysec：204k | 把刷盘挪到后台线程、每秒一次，写吞吐直接回到内存速度，代价是断电或系统崩溃时最多丢约 1 秒 |
...
### O1 AOF 后台刷盘 + 组提交（EVERYSEC 已完成）

- **已完成**：`AofFile` 拆分 `appendAll/force`，`AofPersistence` 根据策略协调刷盘，`EVERYSEC` 使用独立线程每秒检查并刷入新增数据，关闭时对未刷盘数据执行最终刷盘。
- **实测结果**：SET 由同版本 `ALWAYS` 的 272.11 ops/s 提升到 `EVERYSEC` 的 112,359.55 ops/s，约 **413 倍**。
- **尚未完成**：`ALWAYS` 仍是每批命令独占一次刷盘，后续可实现组提交，在不降低持久性语义的前提下让并发请求共享一轮 fsync。
- **恢复验证**：`kill -9` 后AOF可以完整重放；该实验只能覆盖进程崩溃，不能模拟断电导致的页缓存丢失。
- **待补验证**：重跑 Pipeline 与五命令完整矩阵；机器级持久性窗口需要虚拟机或存储故障注入环境。
